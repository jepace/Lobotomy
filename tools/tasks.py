...
def parse_tag(line, prefix):
    """Extract value of a #prefix:value tag, or None."""
    m = re.search(r"#" + re.escape(prefix) + r":(\S+)", line)
    return m.group(1) if m else None


def parse_tasks(text):
    """Parse all tasks from markdown text. Returns list of task dicts."""
    tasks = []
    ...
    while i < len(lines):
        ...
        if task_match:
            ...
            desc = re.sub(r"\s*#\w+:\S+", "", rest).strip()
            task = {
                "done": done,
                "description": desc,
...
def format_task(task, today):
    ...
    meta = "  ".join(x for x in [pri, due_str, ctx, proj] if x)
...
def main():
    ...
    while i < len(args):
        ...
        if a == "--open":
            ...
            filter_context = args[i + 1].lower()
            i += 1
        elif a == "--project" and i + 1 < len(args):
            filter_project = args[i + 1].lower()
            i += 1
        elif a == "--due-today":
            due_today_only = True
        elif a == "--overdue":
    ...
    for t in all_tasks:
        if t["done"] and not show_done:
            continue
        if not t["done"] and not show_open:
            continue
        if filter_priority and t["priority"] != filter_priority:
            continue
        if filter_context and t["context"] != filter_context:
            continue
        if filter_project and t["project"] != filter_project:
            continue
        if due_today_only:
            d = parse_date(t["due"])
            if not d or d > today:
    ...
    # Sort: due date ascending (None last), then priority, then description
