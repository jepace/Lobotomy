...
# Generated and append-only files, shared with agent.py so the passes cannot disagree
# about which pages may be rewritten. See agent._GENERATED_PAGES for why each is on
# the list — in short, nothing can revert a write to either, index.md is regenerated
# anyway, and editing log.md rewrites what the log says happened.
#
# reading-list.md and tasks.md are deliberately NOT here: they are hand-maintained,
# they get history like any other page, and a dead link in them is worth fixing.
_GENERATED = agent._GENERATED_PAGES
